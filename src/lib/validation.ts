...
export const propertyCreateSchema = z.object({
  name: z.string().trim().min(2, "დაარქვი ბინას სახელი.").max(120),
...
export const propertyEmergencySchema = z.object({
  ...
  alert_webhook_url: z
    ...
    .refine(
      ...
      { message: "შეიყვანე სწორი ბმული (https://…)." },
    ),
});

export const applianceSchema = z.object({
  name: z.string().trim().min(2, "დაასახელე ტექნიკა.").max(120),
  brand: optionalText,
  model: optionalText,
  ...
  instructions: z
    .string()
    .trim()
    .min(5, "ახსენი როგორ მუშაობს — სწორედ ამას ეტყვის AI სტუმარს."),
});
...
export const guideSchema = z.object({
  ...
  title: z.string().trim().min(2, "დაარქვი ადგილს სახელი.").max(160),
...
export const upsellSchema = z.object({
  ...
  payment_link: z
    ...
    .refine(
      (value) => value === null || z.url().safeParse(value).success,
      { message: "ჩასვი გადახდის სრული ბმული (https://…)." },
    ),
